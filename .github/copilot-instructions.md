...
- **Clojure**: `guix shell -m manifest.scm -- clojure` で実行（Babashka は guix 環境と非互換のため不使用）
...
| `docs/` | 公開ドキュメント（api.md / test-amplification.md） |
...
| `patch-test-from-mds` | gen-tests/\<class\>/ の per-method md から @Test を既存テストファイルに追記 |
| `disable-failing-tests` | surefire XML を解析して失敗テストに @Disabled を付与 |
