...
`compile-ok-java-files` の戻り値スキーマを確認するための最小 Java サンプルを置いている。

- `SampleOk.java`: コンパイル成功する最小例
- `SampleError.java`: コンパイルエラーになる最小例
...
PC リブート後に `/tmp/tradehub-common-lib-gen-tests.classpath` を再生成する:

```bash
cd /home/mtok/dev.home/openrefine-work/trials/experiments/2026-04-28-tradehub/repo
guix shell 'openjdk@21:jdk' maven -- sh -lc './mvnw -q dependency:build-classpath -pl common-lib -DincludeScope=test -Dmdep.outputFile=/tmp/tradehub-common-lib-test.classpath && cp /tmp/tradehub-common-lib-test.classpath /tmp/tradehub-common-lib-gen-tests.classpath && printf ":%s" "/home/mtok/dev.home/openrefine-work/trials/experiments/2026-04-28-tradehub/repo/common-lib/target/classes" >> /tmp/tradehub-common-lib-gen-tests.classpath && printf "\n" >> /tmp/tradehub-common-lib-gen-tests.classpath'
```
